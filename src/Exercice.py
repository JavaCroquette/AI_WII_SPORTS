            ...
            if self.Video is not None:#On check si il y en a deux
                if self.check:
                    ...
                    if self.Camera > MIN:
                        ...
                        for p in Comparatif:
                            self.sum += (utile.Distance(self.CameraPoint[p],self.VideoPoint[p])/(sqrt(2)*len(Comparatif)))#Normalisation
                    else:
                        self.sum += 1
                    ...
                    print("Camera : " + str(self.Camera_thread.frame_count) +
                          " == Video : " + str(self.Video_thread.frame_count))
    #==============================================================================#
                if (self.Camera_thread.frame_count)%10 == 0:
                    plt.cla()
                    plt.ylim(0,1)
                    ...
                    canvas.draw()
                    self.data = np.frombuffer(self.fig.canvas.tostring_rgb(), dtype=np.uint8)
                    self.data = self.data.reshape(self.fig.canvas.get_width_height()[::-1] + (3,))
                if self.data is not None:
                    self.Video[0:self.data.shape[0], self.Video.shape[1] - self.data.shape[1]:self.Video.shape[1]] = self.data
                    cv2.putText(self.Video, str(MOT_DOUX[self.i]), (50, 200),cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 4)
                    cv2.putText(self.Video, str("Marge d'erreurs:")+str(round(self.listSum[len(self.listSum)-1],2)), (50, 100),cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 0, 0), 4)
    #==============================================================================#
                    img = IMAGE[self.i]
                    self.Video[250:img.shape[0]+250, img.shape[1]+100 -img.shape[1]:img.shape[1]+100] = img
    #==============================================================================#
                self.Video[self.Video.shape[0]-500:self.Video.shape[0], 0:500] = [0, 0, 0]
    #==============================================================================#
                self.Video = utile.draw(self.VideoPoint,self.Video,[0, 255, 255],True)
                if self.Camera > 0:
                    self.Video = utile.draw(self.CameraPoint,self.Video,[255, 255, 0],False)
                else:#Si la personne sort du cadre de la caméra
                    self.Video[:,:] = [100,100,100]
                    cv2.putText(self.Video, str("REVENEZ DEVANT LA CAMERA"), (50, 500),cv2.FONT_HERSHEY_SIMPLEX, 4, (0, 0, 0), 12)
    #==============================================================================#
